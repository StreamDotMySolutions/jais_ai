...
class ComplaintOydObserver
{
    ...
    private function writeLog(ComplaintOyd $record, string $event, array $oldValues = null, array $newValues = null, array $changedKeys = null): void
    {
        $request = request();

        AuditLog::create([
            'user_id' => Auth::id(),
            'auditable_type' => ComplaintOyd::class,
    ...
    public function created(ComplaintOyd $record): void
    {
        $attrs = $record->getAttributes();
        $this->writeLog($record, 'created', null, $attrs, array_keys($attrs));
    }

    public function updated(ComplaintOyd $record): void
    {
    ...
    public function deleted(ComplaintOyd $record): void
    {
        $original = $record->getOriginal();
        $this->writeLog($record, 'deleted', $original, null, array_keys($original));
    }

    public function restored(ComplaintOyd $record): void
    {
